...
async function createPrint(print, path, tableName) {
  ...
  function generatePrintTable(doc, print) {
    ...
    for (i = 0; i < print.lines.length ; i++) {
      ...
      if (j <= linesOnPage) {
        ...
        // ACHTUNG: 002 kann auch bei anderen Belegen kommen - evtl. nur auf CLAL Tabelle prüfen
        ...
        // todo Hier müssen wir anders Abfragen, da es auch Umlagerungen gibt, die ohne eine Frontpage kommen
        // D.h. am Besten auf den Beleg selbst. CLALxxxx
        ...
        else if(item.substring(0,3) == '001'){
          if ( p > 1) {
            j=1;
            dot = 0;
            doc.addPage();
            console.log("Page added.."); 
            p = p+1;
            var s1 = rectanglePosition[1]-rectanglePosition[0] + 1;
            var s2 = rectanglePosition[2]-rectanglePosition[0] + 1;
            doc.rect(50, 50, 250, s1 * fontSizeBold); //9
            doc.rect(50, 50, 500, s1 * fontSizeBold); //9
            doc.rect( 50,50, 500, s2 * fontSizeBold ); //12 18
            doc.stroke();
          ...
          if (parseInt(item.substring(0,3)) > 2 ) {
            if(item.indexOf("KOMMISSIONSKARTE") >= 0){ 
              j = parseInt(item.substring(0,3)) + 3 ;
            }
            else{
        ...
        if( globalTableName.indexOf("IVBG") == -1 ){
          if(item.indexOf("----") > 0){
            dot = dot + 1;
            if (dot == 3){
              j = j + 3;
    ...
    if(p == 0){
      var s1 = rectanglePosition[1]-rectanglePosition[0];
            var s2 = rectanglePosition[2]-rectanglePosition[0];
            doc.rect(50, 50, 250, s1 * fontSizeBold);
            doc.rect(50, 50, 500, s1 * fontSizeBold); 
            doc.rect( 50,50, 500, s2 * fontSizeBold ); 
            doc.stroke();
    }
  ...
  function loadLineDetails(item){
    ...
    if(item.indexOf("DECKEL") > 0){
      item = item.replace('DECKEL','');
    }

    const charArray = item.split("!");

    // todo Menge kommt nicht, weil er das nicht splitten kann. !#13 steht im Text...
//    if (charArray.length == 4 || (charArray.length == 3 && globalTableName.includes("CLAL"))) {
    if (charArray.length == 4) {
      ...
      d.detailFont = fontNormal;
      d.detailFontSize = 12;
      d.detailX = 50; if (item.indexOf("-----") < 0){
        lineDetails.push(d);
      }
      console.log(item);
    }
  ...
  function detailsLoad(item){
    if ((item.indexOf("#13") >=0) && (item.indexOf("#14") >= 0) ){
      ...
      // Wenn indexOf #13 = 0, dann dürfte das knallen. Deshalb filtert er derzeit die Menge beim CLAL oben raus
      let BarcodeX = 0;
      try {
        BarcodeX = item.indexOf("#13") / 82 ;
      } catch (error) {
        BarcodeX = 1 / 82 ;
      }
      ...
      d.detailFontSize = 18;
      if((BeforBC.indexOf("Lagerort" >= 0) || BeforBC.indexOf("Werk  :" >= 0)) && barcode.indexOf("KOMMISSIONSKARTE") < 0){
        d.detailX =  25 + Math.round(350 *  BarcodeX);
      }
      else{
  ...
  function detailsLoad2( item , y){
    if ((item.indexOf("#13") >=0) && (item.indexOf("#14") >= 0) ){
      ...
      console.log(item);
      var d = new LineDetail();
      var beforeCode = item.substring(0 , item.indexOf("#13"));
